...
func (d *LawnMower) UpdateState() {
	if d.ActivityStateTopic != nil {
		state := d.ActivityStateFunc()
		if state != stateStore.LawnMower.ActivityState[d.GetUniqueId()] || (d.MQTT.ForceUpdate != nil && *d.MQTT.ForceUpdate) {
			token := (*d.MQTT.Client).Publish(*d.ActivityStateTopic, byte(*d.Qos), *d.Retain, state)
			stateStore.LawnMower.ActivityState[d.GetUniqueId()] = state
			token.Wait()
		}
	}
	if d.AvailabilityTopic != nil {
		state := d.AvailabilityFunc()
		if state != stateStore.LawnMower.Availability[d.GetUniqueId()] || (d.MQTT.ForceUpdate != nil && *d.MQTT.ForceUpdate) {
			token := (*d.MQTT.Client).Publish(*d.AvailabilityTopic, byte(*d.Qos), *d.Retain, state)
			stateStore.LawnMower.Availability[d.GetUniqueId()] = state
			token.Wait()
		}
	}
}
func (d *LawnMower) Subscribe() {
	...
	if d.DockCommandTopic != nil {
		t := c.Subscribe(*d.DockCommandTopic, 0, d.MQTT.MessageHandler)
		t.Wait()
		if t.Error() != nil {
			log.Fatal(t.Error())
		}
	...
	if d.PauseCommandTopic != nil {
		t := c.Subscribe(*d.PauseCommandTopic, 0, d.MQTT.MessageHandler)
		t.Wait()
		if t.Error() != nil {
			log.Fatal(t.Error())
		}
	}
	if d.StartMowingCommandTopic != nil {
		t := c.Subscribe(*d.StartMowingCommandTopic, 0, d.MQTT.MessageHandler)
		t.Wait()
		if t.Error() != nil {
			log.Fatal(t.Error())
		}
...
func (d *LawnMower) UnSubscribe() {
	...
	token.Wait()
	if d.DockCommandTopic != nil {
		t := c.Unsubscribe(*d.DockCommandTopic)
		t.Wait()
		if t.Error() != nil {
			log.Fatal(t.Error())
		}
	...
	if d.PauseCommandTopic != nil {
		t := c.Unsubscribe(*d.PauseCommandTopic)
		t.Wait()
		if t.Error() != nil {
			log.Fatal(t.Error())
		}
	...
	if d.StartMowingCommandTopic != nil {
		t := c.Unsubscribe(*d.StartMowingCommandTopic)
		t.Wait()
		if t.Error() != nil {
			log.Fatal(t.Error())
		}
...
func (d *LawnMower) PopulateTopics() {
	if d.ActivityStateFunc != nil {
		d.ActivityStateTopic = new(string)
		*d.ActivityStateTopic = GetTopic(d, "activity_state_topic")
	}
	if d.AvailabilityFunc != nil {
		d.AvailabilityTopic = new(string)
		*d.AvailabilityTopic = GetTopic(d, "availability_topic")
	}
	if d.DockCommandFunc != nil {
		d.DockCommandTopic = new(string)
		*d.DockCommandTopic = GetTopic(d, "dock_command_topic")
		store.TopicStore[*d.DockCommandTopic] = &d.DockCommandFunc
	}
	...
	if d.PauseCommandFunc != nil {
		d.PauseCommandTopic = new(string)
		*d.PauseCommandTopic = GetTopic(d, "pause_command_topic")
		store.TopicStore[*d.PauseCommandTopic] = &d.PauseCommandFunc
	}
	if d.StartMowingCommandFunc != nil {
		d.StartMowingCommandTopic = new(string)
		*d.StartMowingCommandTopic = GetTopic(d, "start_mowing_command_topic")
		store.TopicStore[*d.StartMowingCommandTopic] = &d.StartMowingCommandFunc
	}
}
