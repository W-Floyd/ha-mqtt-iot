...
func (d *Humidifier) UpdateState() {
	...
	if d.CurrentHumidityTopic != nil {
		...
		if state != stateStore.Humidifier.CurrentHumidity[d.GetUniqueId()] || (d.MQTT.ForceUpdate != nil && *d.MQTT.ForceUpdate) {
			token := (*d.MQTT.Client).Publish(*d.CurrentHumidityTopic, byte(*d.Qos), *d.Retain, state)
			stateStore.Humidifier.CurrentHumidity[d.GetUniqueId()] = state
			token.Wait()
		}
	}
