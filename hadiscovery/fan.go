...
func (d Fan) UpdateState() {
	if d.AvailabilityTopic != "" {
		...
		if state != stateStore.Fan.Availability[d.UniqueId] || d.MQTT.ForceUpdate {
			c := *d.MQTT.Client
	...
	if d.OscillationStateTopic != "" {
		...
		if state != stateStore.Fan.OscillationState[d.UniqueId] || d.MQTT.ForceUpdate {
			c := *d.MQTT.Client
			token := c.Publish(d.OscillationStateTopic, qos, retain, state)
	...
	if d.PercentageStateTopic != "" {
		...
		if state != stateStore.Fan.PercentageState[d.UniqueId] || d.MQTT.ForceUpdate {
			c := *d.MQTT.Client
			token := c.Publish(d.PercentageStateTopic, qos, retain, state)
